...
#[tauri::command]
pub fn obtener_servicios_genericos() -> Response {
    // Ejecutar el comando para obtener servicios de genericos
    ...
    match output {
        Ok(output) if output.status.success() => {
            ...
            // Aquí puedes filtrar solo servicios obtenidos
            let servicios_genericos: Vec<Service> = services
                .into_iter()
                .filter(|s| s.name.to_lowercase().contains("")) // edita esto para encontrar el conjunto que prefieras
                .collect();

            if servicios_genericos.is_empty() {
                Response::Error {
                    error: true,
                    mensaje: "No se encontraron servicios.".to_string(),
                }
            } else {
                Response::Success {
                    error: false,
                    contenido: servicios_genericos,
                }
            }
        }
