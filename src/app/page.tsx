...
export default function Component() {
  ...
  const fetchServices = useCallback(async () => {
    try {
      const result = await invoke<ApiResponse>('obtener_servicios_genericos')
      if (result.error) {
  ...
  const handleStartStop = async (action: 'start' | 'stop') => {
    if (!selectedService) {
      setPoperError('Debe seleccionar un servicio.')
      return
    }
    ...
    try {
      const response: ApiResponse = await invoke(
        'administrar_servicio_generico',
        {
          action,
  ...
  return (
    <div className="flex flex-col items-center justify-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 sm:pr-[65px] font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        <h1 className="text-3xl font-extrabold text-[hsl(var(--primary)/0.9)] drop-shadow-md">
        ...
        <div className="flex flex-col space-y-2">
          ...
          {poperError ? (
            ...
            >
              El servicio {selectedService.name} está{' '}
              {selectedService.status === '4  RUNNING'
                ? 'iniciado'
                : 'detenido'}
              .
            ...
            <p className="text-sm text-muted-foreground">
              Selecciona el servicio que quieras utilizar o modificar.
            </p>
          )}
        </div>
        ...
        <div className="flex gap-4 items-center flex-col sm:flex-row">
          <Button
            className="shadow-lg shadow-[hsl(var(--primary)/0.9)]/50 rounded-full border border-solid border-transparent text-sm sm:text-base h-10 sm:h-12 px-4 sm:px-5 sm:min-w-44"
